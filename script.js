...
function setupFilters() {
    cols.forEach((col,idx)=>{
        ...
        [...new Set(data.map(r=>r[col]))].sort().forEach(val=>{
            ...
            o.value=o.textContent=val; sel.appendChild(o);
        });
...
function applyFilters() {
    ...
    const filtered = data.filter(r=>cols.every(c=>crit[c]==='Tutti'||r[c]===crit[c]));
...
function displayCards(list) {
    ...
    list.forEach(r=>{
        ...
        card.innerHTML = `
            ...
            <p class="commission">
                Tel: ${r.Telefono} – Budget: €${r["Budget (€)"]} – Costo: ${r["Costo (crediti)"]} crediti
            </p>
            <div class="actions">
...
function addToCart(id,cred) {
    if(!carrello.find(x=>x.id===id)) carrello.push({id,cred}), updateCart();
}
function removeFromCart(id) {
    carrello=carrello.filter(x=>x.id!==id), updateCart();
}
function updateCart() {
