document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnRegister').onclick = () => {
    ...
    if (cap !== '5')   return alert('Captcha non corretto');
  ...
  elems.sendContact.onclick = () => {
    ...
    if (!n||!s||!t||!l||!m||!msg) return alert('Compila tutti i campi del contatto');
  ...
  fetch(sheetURL).then(r => r.text()).then(txt => {
    ...
    leads = lines.map(l => {
      const c = l.split('\t');
      return {
        regione: c[0]||'', citta: c[1]||'',
        categoria: c[2]||'', tipo: c[3]||'',
        descrizione: c[4]||'', budget: parseFloat(c[6])||0
      };
    });
    populateFilters();
    render();
