...
}function removeFromCart(id){carrello=carrello.filter(x=>x.id!==id),updateCart(); displayPhone(id);}function updateCart(){document.getElementById('carrello').innerHTML=carrello.map(x=>`<li>${x.id} – ${x.cred} crediti</li>`).join('');document.getElementById('totale').textContent=`Totale: €${carrello.reduce((s,x)=>s+x.cred,0)}`;}function resetFilters(){['regione','citta','categoria','tipo'].forEach(id=>document.getElementById(id).value='Tutti');drawCards(data);}function filterByCategoria(cat){drawCards(data.filter(r=>r.Categoria.includes(cat)));}function openRicarica(){alert('Apri ricarica PayPal');}
