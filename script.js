...
function populateFilters(){
  ...
  Object.entries(map).forEach(([field,selId])=>{
    ...
    [...new Set(allLeads.map(l=>l[field]))].filter(v=>v).sort().forEach(v=>{
...
function setSection(sec){
  ...
  document.getElementById({lead:'btnLeads',appuntamento:'btnAppuntamenti',contratto:'btnContratti'}[sec]).classList.add('selected');
  render();
}

function render(){
  const r=document.getElementById('filter-region').value,
        c=document.getElementById('filter-city').value,
        cat=document.getElementById('filter-category').value,
        t=document.getElementById('filter-type').value;
  const cont=document.getElementById('clienti');
  cont.innerHTML='';
  const filtered = allLeads.filter(l=>
    l.tipo===sectionFilter &&
    (!r||l.regione===r)&&(!c||l.città===c)&&(!cat||l.categoria===cat)&&(!t||l.tipo===t)
  );
  if(!filtered.length){ cont.innerHTML='<p>Nessun risultato</p>'; return; }
  filtered.forEach(l=>{
    ...
    const a=card.querySelector('.acquisisci'),
          rbtn=card.querySelector('.riserva');
    if(a) a.onclick=()=>{ cart.push(l); updateCart(); toggleModal('payment-modal',true); };
    if(rbtn) rbtn.onclick=()=>toggleModal('contact-modal',true);
