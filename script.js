...
function render() {
  ...
  leads.forEach(lead => {
    ...
    if (lead.costCredit > 0) {
      ...
      btnA.className = lead.costCredit>1?'appuntamento':'acquisisci';
      ...
      btnR.className = 'contratto';
      btnR.textContent = 'Riserva trattativa';
      act.append(btnR);
    }
    card.append(act);
    ...
    .then(txt => {
      ...
      leads = lines.map((l,i) => {
        ...
        return {
          ...
          telefono: c[idx.telefono]||'',
          budget: parseInt(c[idx.budget])||0,
          costCredit: isNaN(parseInt(c[idx.costCredit]))?0:parseInt(c[idx.costCredit])
        };
      });
