document.addEventListener('DOMContentLoaded', () => {
  ...
  // Register, login, recover handlers...
  ...
  fetch(sheetURL).then(r => r.text()).then(txt => {
    ...
    leads = lines.map((l,i) => {
      ...
      return {
        ...
        budget: parseFloat(c[idx.budget])||0
      };
    });
    populateFilters();
    render();
  ...
  function updateCart() {
    ...
    carrello.forEach(item => {
      ...
      const total = cost * 40; sum += total;
      const li = document.createElement('li');
      li.textContent = `${item.categoria} – €${total}`;
