...
function resetFilters() {
  ['regione','citta','categoria','tipo'].forEach(id => {
    document.getElementById(id + 'Filter').selectedIndex = 0;
  });
  setActiveTab('');
  displayCards(data);
}
...
function aggiornaCarrello() {
  ...
  document.getElementById('totale').textContent = `Totale: €${carrello.reduce((s, x) => s + x.crediti, 0)}`;
}
function openRicarica() {
  // Apri il modal di PayPal
}