...
window.addEventListener('DOMContentLoaded', () => {
  fetch(sheetURL)
    .then(res => res.text())
    .then(parseTSV)
    .then(parsed => { data = parsed; populateFilters(); resetFilters(); })
    .catch(console.error);
});
...
function parseTSV(tsv) {
  ...
  return lines.map(line => Object.fromEntries(headers.map((h,i)=>[h.trim(), line.split('\t')[i]?.trim()||''])));
}
...
function resetFilters() {
  ['regione','citta','categoria','tipo'].forEach(id=>document.getElementById(id+'Filter').selectedIndex=0);
  setActiveTab('');
  displayCards(data);
}
function filterByCategoria(cat) { setActiveTab(cat); displayCards(data.filter(e=>e.Categoria===cat)); }
function applyFilters() {
  const [r,c,ca,t] = ['regione','citta','categoria','tipo'].map(id=>document.getElementById(id+'Filter').value);
  setActiveTab('');
  displayCards(data.filter(e=>
...
function displayCards(list) {
  ...
  list.forEach(e=>{
    const div=document.createElement('div'); div.className='cliente-card '+e.Categoria.toLowerCase();
    div.innerHTML=`
      <span class="badge ${e.Categoria.toLowerCase()}">${e.Categoria}</span>
      <h3>${e.Tipo}</h3>
      <p class="desc">${e.Descrizione}</p>
      <p><strong>${e.Città}, ${e.Regione}</strong></p>
      <p class="commission">Tel: ${e.Telefono} – Budget: €${e["Budget (€)"]} – Costo: ${e["Costo (crediti)"]} crediti</p>
      <div class="actions">
        <button class="acquisisci ${e.Categoria.toLowerCase()}" onclick="aggiungiACarrello('${e.Telefono}',${e["Costo (crediti)"]})">Acquisisci</button>
        <button class="annulla" onclick="rimuoviDalCarrello('${e.Telefono}')">Annulla</button>
      </div>`;
    cont.appendChild(div);
  });
}
function aggiungiACarrello(id, crediti){if(!carrello.find(x=>x.id===id)){carrello.push({id, crediti}); aggiornaCarrello();}}
function rimuoviDalCarrello(id){carrello=carrello.filter(x=>x.id!==id); aggiornaCarrello();}
function aggiornaCarrello(){ 
  document.getElementById('carrello').innerHTML = carrello.map(x=>`<div>${x.id} – ${x.crediti} crediti</div>`).join('');
  document.getElementById('totale').textContent = `Totale: €${carrello.reduce((sum,x)=>sum+x.crediti,0)}`; 
}
